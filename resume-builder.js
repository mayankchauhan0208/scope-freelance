(function (root, factory) {
  ...
  function initBuilder() {
    ...
    let current=null; const render=()=>{if(!current)return;root.document.querySelector('#atsScore').textContent=current.analysis.score;root.document.querySelector('#atsScoreLabel').textContent=current.analysis.score>=80?'Strong':current.analysis.score>=60?'Review':'Needs work';root.document.querySelector('#atsSectionScores').innerHTML=Object.entries(current.analysis.factors).map(([key,value])=>`<span><b>${value}/${current.analysis.max[key]}</b>${key.replace(/([A-Z])/g,' $1')}</span>`).join('');root.document.querySelector('#atsIssues').innerHTML=current.analysis.issues.length?current.analysis.issues.map(issue=>`<li>${root.ScopeSecurity.escapeHtml(issue)}</li>`).join(''):'<li>No major rule-based issues detected. Manual review is still required.</li>';root.document.querySelector('#atsMissingKeywords').textContent=current.analysis.missingKeywords.length?`Missing keywords: ${current.analysis.missingKeywords.join(', ')}`:'No obvious target keywords are missing.';root.document.querySelector('#atsImproved').value=current.generated;root.document.querySelector('#atsTruthWarning').textContent=current.analysis.truthWarnings.join(' ')};
    const analyze=()=>{const text=original.value.trim();if(text.length<80){root.toast('Paste more resume content before analysis');return}const extracted=extractProfile(text,currentProfile()),target=fieldValue('atsTargetRole')||extracted.targetRole,tone=fieldValue('atsTone')||'Corporate';current={extracted,analysis:analyzeAts(text,extracted,target),generated:generateAtsResume(extracted,tone,target),originalText:text,targetRole:target,tone};root.document.querySelector('#atsBuilderStatus').textContent='Analysis complete. Review every field and warning.';render()};
